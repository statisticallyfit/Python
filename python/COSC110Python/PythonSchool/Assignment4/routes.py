...
def isValidRoute(routeStr, ROUTE_RECORD):
    ...
    if isValidPosInt(routeStr):
        route = int(routeStr)
        if isUniqueRoute(route, ROUTE_RECORD):
...
def isValidRatio(ratioStr):
    ...
    # check exactly 1 decimal point AND there should be at least 1 digit before decimal AND at most 2 decimal places.
    isIntStyle = len(firstSecond) == 1
    isFloatStyleAndAtMostTwoDecimals = len(firstSecond) == 2 and len(firstSecond[0]) > 0 and len(firstSecond[1]) <= 2
    return isIntStyle or isFloatStyleAndAtMostTwoDecimals




def isValidInput(inputLine, ROUTE_RECORD):
    ...
    if len(routeRatio) == 2:
        ...
        return isValidRoute(routeStr, ROUTE_RECORD) and isValidRatio(ratioStr)
...
def isUniqueRoute(targetRoute, ROUTE_RECORD):
    ...
    '''
    if not isinstance(targetRoute, int):
        return False
...
def readRouteData(fileName):
    ...
    fileIn = open(fileName, "r") # fixed - teacher said no point try/except here because we do that in main
    # fixed - here, just let the error happen and we will shuttle out of the function with it.
    ...
    for aLine in fileIn:
        ...
        if not isValidInput(line, ROUTE_RECORD):
...
def swap(dct, x, y):
    '''Swaps the dictionary values so that at index x we get index y's value
     ...
     '''
    # fixed this first line is wrong - was switching happy ratios not the entire
    # fixed dictionaries!!!!
    #dct[x]['happyRatio'], dct[y]['happyRatio'] = dct[y]['happyRatio'], dct[x]['happyRatio']
    dct[x], dct[y] = dct[y], dct[x]
...
if __name__ == "__main__":
    ...
    data = []
    try:
        data = readRouteData("routes_sample_3.txt")
